...
- Deduplication: if a learning file with the task name already exists in `learnings/failures/`, the runtime does not overwrite it
...
- The `## Generalizable Learning` section must not simply restate the failure report's summary or path-to-resolution. It must identify the *class* of failure (e.g., "specs that require modifying human-only files create unresolvable blockers for agents") and a preventive recommendation.
...
- Retroactive learning extraction from existing failure reports that predate this convention (though manually adding the section to existing reports would allow the runtime to extract them).
