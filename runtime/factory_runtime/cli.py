...
def _generate_cli_skill() -> str:
    """Generate a SKILL.md document from the live click command tree.

    Introspects every command registered on ``main``, extracts arguments,
    options, and docstrings, then renders them into the standard factory
    ...
    # ── Frontmatter ──
    lines.append("---")
    lines.append("name: factory-cli")
    lines.append("description: Complete reference for the factory CLI — all commands, arguments, options, and usage.")
    lines.append("---")
    lines.append("")
    lines.append("# Factory CLI Reference")
    lines.append("")
    lines.append("Auto-generated by `factory update-claude-skill`. Do not hand-edit.")
    lines.append("")

    # ── Top-level help (the categorized overview) ──
    ctx = click.Context(main, info_name="factory")
    lines.append("## Overview")
    lines.append("")
    lines.append("```")
    lines.append(main.get_help(ctx).rstrip())
    lines.append("```")
    lines.append("")
    ...
    for cmd_name in commands:
        ...
        cmd_ctx = click.Context(cmd, info_name=cmd_name, parent=ctx)

        # Command heading
        short = cmd.get_short_help_str(limit=300)
        lines.append(f"### factory {cmd_name}")
        lines.append("")
        ...
        # Usage + full help
        lines.append("```")
        lines.append(cmd.get_help(cmd_ctx).rstrip())
        lines.append("```")
        lines.append("")
        ...
        if isinstance(cmd, click.Group):
            ...
            for sub_name in cmd.list_commands(sub_ctx):
                ...
                lines.append(f"#### factory {cmd_name} {sub_name}")
                lines.append("")
                lines.append(f"{sub_short}")
                lines.append("")
                lines.append("```")
                lines.append(sub_cmd.get_help(sub_cmd_ctx).rstrip())
                lines.append("```")
                lines.append("")
...
def update_claude_skill() -> None:
    """Generate skills/shared/factory-cli/SKILL.md from the current CLI.

    Introspects every registered command, extracts arguments, options,
    and help text, and writes a complete CLI reference as a factory skill.
    Run this after any CLI changes to keep the skill in sync.
    ...
    content = _generate_cli_skill()
    skill_file.write_text(content)

    console.print(f"[green]Wrote:[/green] {skill_file.relative_to(workspace)}")
    console.print(f"  {len(content)} bytes, {content.count(chr(10))} lines")
