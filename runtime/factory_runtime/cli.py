...
# ── CLI Skill Generator ──
# Command-to-category mapping.  The generator uses this to group commands
# under semantic XML tags in the references file.  Keep in sync with the
# top-level group docstring above.
...
def _cmd_help_block(cmd: click.Command, cmd_name: str, parent_ctx: click.Context) -> str:
    """Render the full help text for a single command as an XML block."""
    cmd_ctx = click.Context(cmd, info_name=cmd_name, parent=parent_ctx)
    short = cmd.get_short_help_str(limit=300)
    full_help = cmd.get_help(cmd_ctx).rstrip()

    block = f'<command name="factory {cmd_name}" short="{short}">\n'
    block += f"```\n{full_help}\n```\n"

    # If it's a group, enumerate subcommands
    if isinstance(cmd, click.Group):
        sub_ctx = click.Context(cmd, info_name=cmd_name, parent=parent_ctx)
        for sub_name in cmd.list_commands(sub_ctx):
            sub_cmd = cmd.get_command(sub_ctx, sub_name)
            if sub_cmd is None:
                continue
            sub_cmd_ctx = click.Context(sub_cmd, info_name=sub_name, parent=sub_ctx)
            sub_short = sub_cmd.get_short_help_str(limit=300)
            sub_help = sub_cmd.get_help(sub_cmd_ctx).rstrip()
            block += f'\n<subcommand name="factory {cmd_name} {sub_name}" short="{sub_short}">\n'
            block += f"```\n{sub_help}\n```\n"
            block += "</subcommand>\n"
...
def _generate_cli_skill() -> str:
    """Generate SKILL.md — the compact skill with overview, quick_start, and pointers."""
    ctx = click.Context(main, info_name="factory")
    overview_help = main.get_help(ctx).rstrip()
...
def _generate_cli_references() -> str:
    """Generate references/commands.md — full per-command reference grouped by category."""
    ctx = click.Context(main, info_name="factory")

    lines: list[str] = []
    lines.append("<!-- Auto-generated by `factory update-claude-skill`. Do not hand-edit. -->")
    lines.append("")

    category_labels = {
        "operate": "Operate — pipeline work",
        "inspect": "Inspect — read-only views",
        "configure": "Configure — setup and settings",
        "gc": "GC — garbage collection (also automatic after every agent run)",
    }

    for cat_key, cat_cmds in _CMD_CATEGORIES.items():
        label = category_labels.get(cat_key, cat_key)
        lines.append(f'<{cat_key}_commands label="{label}">')
        lines.append("")
        ...
        for cmd_name in cat_cmds:
            ...
            if cmd is None:
                continue
            lines.append(_cmd_help_block(cmd, cmd_name, ctx))

        lines.append(f"</{cat_key}_commands>")
        lines.append("")

    return "\n".join(lines) + "\n"


@main.command(name="update-claude-skill", short_help="Regenerate CLI reference skill from live commands")
def update_claude_skill() -> None:
    """Generate skills/shared/factory-cli/ from the current CLI.

    Writes two files:
      SKILL.md              Compact overview with required XML tags
      references/commands.md  Full per-command reference grouped by category

    Introspects every registered command, extracts arguments, options,
    and help text. Run this after any CLI changes to keep the skill in sync.
    ...
    workspace = config.workspace
    skill_dir = workspace / "skills" / "shared" / "factory-cli"
    refs_dir = skill_dir / "references"
    skill_dir.mkdir(parents=True, exist_ok=True)
    refs_dir.mkdir(parents=True, exist_ok=True)

    skill_file = skill_dir / "SKILL.md"
    refs_file = refs_dir / "commands.md"

    skill_content = _generate_cli_skill()
    refs_content = _generate_cli_references()

    skill_file.write_text(skill_content)
    refs_file.write_text(refs_content)

    skill_lines = skill_content.count("\n")
    refs_lines = refs_content.count("\n")

    console.print(f"[green]Wrote:[/green] {skill_file.relative_to(workspace)}")
    console.print(f"  {len(skill_content)} bytes, {skill_lines} lines")
    console.print(f"[green]Wrote:[/green] {refs_file.relative_to(workspace)}")
    console.print(f"  {len(refs_content)} bytes, {refs_lines} lines")
