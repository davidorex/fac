---
name: factory-cli
description: Complete reference for the factory CLI — all commands, arguments, options, and usage.
---
...
Advance a drafting spec with gathered context
...
Registered workflow apps and pipelines
...
Switch all agents between kimi and anthropic
...
### factory cleanup-factory-internal

Remove promoted/dismissed observations
...
### factory cleanup-research

Remove stale research requests (completed)
...
### factory cleanup-specs

Remove stale specs (upstream copies)
...
### factory cleanup-tasks

Remove stale review tasks (verified/failed)
...
Respond to hard decision gates
...
Update docs after spec completes pipeline
...
Register cwd as a factory project
...
Recent run logs (per-agent or all)
...
Agent blockers and pending human requests
...
Unix-style permissions (groups, modes, escalation)
...
Re-queue failed work with failure report
...
Mark a failed task as resolved (no rebuild)
...
Invoke a specific agent (heartbeat if no task)
...
Manage verification scenario holdouts
...
Automated dispatch loop (downstream-first)
...
Pipeline monitor — agents, queues, next actions
...
### factory update-claude-skill

Regenerate CLI reference skill from live commands
...
Usage: factory update-claude-skill [OPTIONS]
  ...
  text, and writes a complete CLI reference as a factory skill. Run this after
  any CLI changes to keep the skill in sync.
...
Directory tree with item counts
